...
for iSide = 1:2
    ...
    for iSess = 1:3
        ...
        for iSubject=1:length(Trials_Info)
            ...
            [stats_valid, stats_invalid, stats_nochange, stats_cue, rt_all] = process_datablock(all_blocks, var_type,rt_means{iSubject});
...
function [stats_valid, stats_invalid, stats_nochange, stats_cue, rt_all] = process_datablock(all_blocks, var_type, rt_mean)
...
for iBlock = 1:5
    ...
    rt = ablock(startTrial:end,10)./rt_mean; %without normalizing RT
    
    %Normalization: Divide by subject mean then take log.
    for trl=1:length(rt)
        if rt(trl)~=0
            rt(trl)=log(rt(trl));
        end
...
[stats_valid]  = est_and_corr(valid_rep_all, params, rt_all, ptraj_valid,ptraj_valid_grid);
[stats_invalid]  = est_and_corr(invalid_rep_all, params, rt_all, ptraj_invalid,ptraj_invalid_grid);
[stats_nochange]  = est_and_corr(nochange_rep_all, params, rt_all, ptraj_nochange,ptraj_nochange_grid);
[stats_cue]  = est_and_corr(cue_rep_all, params, rt_all, ptraj_cue,ptraj_cue_grid);
...
%                 ptraj_pred_grid{aa,pp,sc} = ptraj_grid{aa,pp,sc};
                ...
                %For getting correlation only for when the event was 1, may
                %have misinterpreted what sir meant here. Saved as 'compareGridbest...rForEvent1...'
                %[r, p] = corrcoef(ptraj_pred_grid{aa,pp,sc}(rt~=0 & bin_vec==1), rt(rt~=0 & bin_vec==1));
                
                if numel(r)>1
                    rcache(aa,pp,sc) = r(1,2); pcache(aa,pp,sc) = p(1,2);
                else
                    rcache(aa,pp,sc) = NaN; pcache(aa,pp,sc) = NaN;
                end
            end
        end
    end
    ...
    [~, idx] = nanmin((rcache(:))); % max r (should correspond to most significant too).
